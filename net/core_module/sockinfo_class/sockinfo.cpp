...
bool kanistra::net::sockinfo::operator==(const sockaddr_in& addr) {
    bool addreq = this->addr   == addr.sin_addr;
    bool porteq = this->port   == addr.sin_port;
    bool fameq  = this->family == addr.sin_family;

    return addreq && porteq && fameq;
}
...
void kanistra::net::sockinfo::set_addr(const kanistra::net::ip& addr)          { this->addr = addr; this->family = addr.get_family(); }
